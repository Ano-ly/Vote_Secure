    ...
    <div className='w-full flex justify-between items-center gap-3 px-4 py-3'>
      ...
      <div className='main-hero-bg shadow-inner p-1 flex justify-start items-start rounded-3xl w-1/6'>
        <div className='lg:w-1/3 w-full'>
          <img className='lg:w-2/3 w-full' src={profilepic} alt="Profile" />
        </div>
        <div className='w-2/3 hidden lg:flex flex-col justify-start'>
          {/* Show username if available, otherwise show "Guest" */}
          <p className='text-white text-opacity-40'>
            {username && username.trim() !== '' ? username : 'Guest'}
          </p>
          <button className='ring-2 px-2 rounded-lg w-1/2 text-left text-blue-800 ring-blue-800'>
            free
          </button>
        </div>
      </div>
